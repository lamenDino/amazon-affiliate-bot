...
def extract_price(soup) -> str:
    ...
    price_selectors = [
        {'class': 'a-price-whole'},
        {'class': 'a-price-symbol a-currency-symbol'},
    ...
    try:
        ...
        price_elem = soup.find('div', {'data-a-color': 'price'})
        if price_elem:
            price_text = price_elem.get_text(strip=True)
